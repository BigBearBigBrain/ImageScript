...
export class Image {
  ...
  *iterateWithColors() {
  ...
  contain(width, height, mode = Image.RESIZE_NEAREST_NEIGHBOR) {
    const scaleFactor = width / height > this.width / this.height
      ? height / this.height
      : width / this.width;
  ...
  composite(source, x = 0, y = 0) {
    ...
    for (let yy = 0; yy < source.height; yy++) {
      ...
      for (let xx = 0; xx < source.width; xx++) {
        ...
        } else {this.__view__.setUint32(
            offset,
            Image.__alpha_blend__(fg, bg),
            false,
          );}
      }
    }
    ...
    if (ImageType.isPNG(view)) { // PNG
      ...
      if (pixelType === 0) {
        ...
        for (let i = 0; i < buffer.length; i += 4) {
          image.bitmap[i] = 0xff * (1 - buffer[i] / 0xff) *
            (1 - buffer[i + 3] / 0xff);
          image.bitmap[i + 1] = 0xff * (1 - buffer[i + 1] / 0xff) *
            (1 - buffer[i + 3] / 0xff);
          image.bitmap[i + 2] = 0xff * (1 - buffer[i + 2] / 0xff) *
            (1 - buffer[i + 3] / 0xff);
  ...
  static async renderSVG(svg, size = 1, mode = this.SVG_MODE_SCALE) {
    if (
      ![this.SVG_MODE_WIDTH, this.SVG_MODE_HEIGHT, this.SVG_MODE_SCALE]
        .includes(mode)
    ) {
...
export class TextLayout {
  ...
  constructor(options) {
    ...
    this.wrapHardBreaks = typeof wrapHardBreaks === "undefined"
      ? true
      : wrapHardBreaks;
