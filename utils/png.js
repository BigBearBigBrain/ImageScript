...
import { compress, decompress } from "../png/src/zlib.mjs";
...
export async function encode(
  data,
  { width, height, channels, depth = 8, level = 0 },
) {
  let offset = 0;
  let tmp_offset = 0;
...
function filter_3(slice, pixels, p_offset, bytespp, row_length) {
  ...
  if (0 === p_offset) {
    ...
    while (i < row_length) {
      pixels[i + p_offset] = slice[i] +
        (pixels[i + p_offset - bytespp] + pixels[i++ + p_offset - row_length] >>
          1);
    }
  }
}
