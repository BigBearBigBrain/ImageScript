...
export default {
  ...
  new() {
    ...
    class Font {
      ...
      has(char) {
        return wasm.font_has(
          this.ptr,
          String.prototype.charCodeAt.call(char, 0),
        );
      }

      metrics(char, scale = this.scale) {
        const ptr = wasm.font_metrics(
          this.ptr,
          String.prototype.charCodeAt.call(char, 0),
          scale,
        );
        const metrics = JSON.parse(
          decode_utf8(mem.u8(wasm.font_metrics_buffer(ptr), mem.length())),
        );
      ...
      rasterize(char, scale = this.scale) {
        ...
        const glyph = {
          ...
          metrics: JSON.parse(
            decode_utf8(mem.u8(wasm.font_rasterize_metrics(ptr), mem.length())),
          ),
        };
    ...
    class Layout {
      ...
      append(font, text, init) {
        ...
        const has_color = ("r" in options) || ("g" in options) ||
          ("b" in options);
        wasm.layout_append(
          this.ptr,
          font.ptr,
          ptr,
          text.length,
          options.scale == null ? font.scale : options.scale,
          has_color,
          options.r,
          options.g,
          options.b,
      ...
      rasterize(r, g, b) {
        ...
        const framebuffer = {
          ...
          buffer: mem.u8(wasm.layout_rasterize_buffer(ptr), mem.length())
            .slice(),
        };
